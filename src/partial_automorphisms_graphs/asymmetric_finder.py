...
from typing import Dict
import copy
from partial_automorphisms_graphs import Graph
from ast import literal_eval
import networkx as nx
import random
...
# ASYM_G_DB_PATH = 'C:\local_repos\symmetries_final\partial_automorphisms_graphs/all_graphs/asymmetric/asym_graphs.db'
# ASYM_G_DB_PATH = '/home/sage/repository/partial_symmetries/partial_automorphisms_graphs/all_graphs/asymmetric/asym_graphs.db'
ASYM_G_DB_PATH = 'projects/partial_symmetries/partial_symmetries/partial_automorphisms_graphs/all_graphs/asymmetric/asym_graphs.db'
ASYM_G_DB_PATH = '/home/p/pastorek20/projects/partial_symmetries/partial_symmetries/partial_automorphisms_graphs/all_graphs/asymmetric/asym_graphs.db'
...
def add_vertex_to_asymmetric(add_to_vertices, from_depth, to_depth):
    with open('partial_automorphisms_graphs/all_graphs/asymmetric/asym_d_' + str(from_depth) + '_' + str(add_to_vertices) + 'v.txt') as file:
        ...
        for g in file:
            ...
            for no_neighbours in range(len(data) + 1):
                for combs in itertools.combinations(data.keys(), no_neighbours):
                    ...
                    if has_asymmetricity_level(Graph(nx.from_dict_of_lists(data_copy)), to_depth):
                        with open('partial_automorphisms_graphs/all_graphs/asymmetric/asym_d_' + str(to_depth) + '_' + str(add_to_vertices + 1) + 'v.txt', 'a') as output:
...
def find_asym_d_random(n, only_save_depth_gte=4, lock=None):
    ...
    while True:
        ...
        if asym_d_max >= only_save_depth_gte:
            ...
            try:
                with open('partial_automorphisms_graphs/all_graphs/asymmetric/random_maximal_asym_d.txt', 'a') as file:
