...
def read_graph_from_file():
    graphs = []
    with open('partial_automorphisms_graphs/tests/inputs.txt', 'r') as file:
...
def partial_symmetries_info(vertices, include_complete=True):
    with open('partial_automorphisms_graphs/all_graphs/data/all_graphs_' + str(vertices) + 'n.txt') as file:
            file = file.read().split('\n')
...
def run_tests(runs=3, clear_output_file=False):
    ...
    if clear_output_file:
        with open('partial_automorphisms_graphs/tests/output.txt', 'w') as file:
            file.write('')
    for graph in read_graph_from_file():
        ...
        for i in range(runs):
            ...
            if i != runs - 1:
                p.clear()
        with open('partial_automorphisms_graphs/tests/output.txt', 'a') as file:
    ...
    for g in read_gap_file(n):
        if not found:
            if g.data == {1: {6, 8, 9}, 2: {7, 9, 10}, 3: {7}, 4: {8, 9, 10}, 5: set(), 6: {1, 10}, 7: {2, 3, 8, 10}, 8: {1, 4, 7, 9, 10}, 9: {1, 2, 4, 8}, 10: {2, 4, 6, 7, 8}}:
                found = True
            continue
        with open('partial_automorphisms_graphs/data/all_graphs_' + str(n) + 'n.txt', 'a') as file:
